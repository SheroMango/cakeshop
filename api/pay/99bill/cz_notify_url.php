...
if ($ok == 1) { 
	switch($_REQUEST[payResult]){
		case '10':
			...
			$rtnUrl=$url."payment/99bill/cz_show_url.php?msg=success";
			break;
		default:
			//false
			$rtnOK=0;
			$rtnUrl=$url."payment/99bill/cz_show_url.php?msg=false";
			break;	
	}
}else{
	//error
	$rtnOK=0;
	$rtnUrl=$url."payment/99bill/cz_show_url.php?msg=error";
}
?>
